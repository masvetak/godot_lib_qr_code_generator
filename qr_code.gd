...
func _encode_numeric(value: String) -> void:
	...
	for index in data.size():
		...
		qr_data_list.append_array(QRCodeUtils.convert_to_binary(data[index], size))
...
func _encode_alphanumeric(value: String) -> void:
	...
	for index in range(0, ceil(value.length() / 2.0)):
		...
		if index * 2 + 1 < value.length():
			second_value = QRCodeUtils.ALPHANUMERIC_CHARACTERS[value[index * 2 + 1]]
		
		var result: Array
		if second_value != -1:
			result = QRCodeUtils.convert_to_binary(first_value * 45 + second_value, 11)
		else:
			result = QRCodeUtils.convert_to_binary(first_value, 6)
...
func _set_info_segments() -> void:
	...
	var length_bits = QRCodeUtils.convert_to_binary(qr_data_length, length_bits_size)
...
func _set_error_correction() -> void:
	...
	for block_index in range(qr_data_list.size()):
		...
		for index in block_bytes.size():
			block_bytes[index] = QRCodeUtils.convert_to_decimal(block_bytes[index])
		
		block_bytes = rs.get_remainder(block_bytes)
		
		for byte in block_bytes:
			ecc_data_list[block_index].append_array(QRCodeUtils.convert_to_binary(byte, 8))
